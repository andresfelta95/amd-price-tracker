  ...
  return (
    <div className="bg-amd-gray border border-gray-700 rounded-xl p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-white font-semibold text-lg">Canadian Retailers (CAD)</h3>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-700">
              <th className="text-left text-gray-400 text-sm font-medium pb-3">Retailer</th>
              <th className="text-left text-gray-400 text-sm font-medium pb-3">Price</th>
              <th className="text-left text-gray-400 text-sm font-medium pb-3">Stock</th>
              <th className="text-right text-gray-400 text-sm font-medium pb-3">Action</th>
            </tr>
          </thead>
          <tbody>
            {sorted.map((retailer, idx) => (
              <tr key={retailer.name} className="border-b border-gray-800 last:border-0">
                <td className="py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white font-medium">{retailer.name}</span>
                  </div>
                </td>
                <td className="py-3">
                  <span className={`font-semibold text-lg ${idx === 0 ? "text-green-400" : "text-white"}`}>
      ...
      {lastChecked && (
        <p className="text-gray-500 text-xs mt-3">
          Last checked: {new Date(lastChecked).toLocaleString("en-CA")}
        </p>
      )}
    </div>
  );
